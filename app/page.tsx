...
export default function Home() {
  ...
  return <main className="atlas">
    ...
    <nav className="mobile-day-switcher" aria-label="切换行程日期" style={leafVariables(dayId) as CSSProperties}>
      ...
      <div className="mobile-day-current" aria-live="polite"><span className="mobile-day-meta"><span className="mobile-day-leaf" aria-hidden="true"><img src="/autumn-birch-384.webp" alt=""/></span><span>DAY {String(day.id).padStart(2,'0')} · {day.date}</span></span><strong>{day.short}</strong></div>
    ...
    <details className="data-note"><summary>地图与数据说明</summary><p>驾车轨迹来自 2026 年 9 月 9 日百度地图 MCP 查询。默认河湖简图随页面提供，采用 Natural Earth 1:50m 公有领域数据，仅帮助辨认大致地理关系，不含完整道路与行政边界；详细底图按需连接 OpenStreetMap。显示坐标从百度 BD-09 转为 WGS84，导航链接仍使用原始 BD-09。此页用于路线讨论，不替代行驶中的实时导航。时间不含休息、游览、补给及节假日额外拥堵，租车门店和住宿入口确定后需重新核算。</p><p>每日头图按当日沿线区域或出发、到达城市切换。每张图附独立出处与历史时间，夏季照片会单独标明；照片不意味着已安排相应景点入园。第六天采用返程出发地阿尔山市区，机场／西站与是否住松原都不改变这一出发地。</p><p>叶片颜色是依据区域景观设计的季节氛围，不是物候观测值、今年红叶预报或最佳观赏期保证；未把“入秋推迟”作为已核实的全线结论。草原、白桦、落叶松的变化不能用单一红叶百分比概括。满洲里到伊尔施按右旗、左旗锁定；直接在百度重新规划全段可能返回另一条路线。</p></details>
  </main>;
}
