...
import { getDays, segments, routeStops } from './trip';
import { DailyStay, BookedStays } from './booked-stays';
...
export default function Home() {
  ...
  const days=useMemo(()=>getDays(),[]),day=days[dayId-1];
  ...
  const fallback=segments.red_airport.destination;
  ...
  useEffect(()=>{
    ...
    const tool:Tool={name:'configure_trip_preview',description:'Select a day in the confirmed hotel itinerary. Only changes the displayed day; does not book or navigate a car.',inputSchema:{type:'object',properties:{day:{type:'integer',minimum:1,maximum:7}},required:['day'],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:false},execute(input){
      if(!input||typeof input!=='object')throw new Error('Expected an object');
      const value=input as Record<string,unknown>;
      if(Object.keys(value).some(k=>k!=='day')||!Number.isInteger(value.day)||Number(value.day)<1||Number(value.day)>7)throw new Error('day must be integer 1–7; hotels are fixed');
      flushSync(()=>{setDayId(Number(value.day));setPlaying(false);setProgress(0);setNavOpen(false);setOverview(false);setSelectedSceneId(null);});
      return {day:Number(value.day),state:'preview_updated',itinerary:'confirmed_hotels_20260920'};
    }};
  ...
  return <main className="atlas">
    ...
    <header className="site-header"><a className="brand" href="/" aria-label="北纬秋行首页"><TreeDeciduous size={32}/><span>北纬秋行<small>AUTUMN ROAD ATLAS</small></span></a><span className="trip-date">2026.09.26 — 10.03</span><div className="header-meta"><Button variant="ghost" className="atmosphere-button" aria-label={atmosphere?'关闭交互叶片与秋日装饰':'开启交互叶片与秋日装饰'} aria-pressed={atmosphere} onClick={()=>setAtmosphere(v=>!v)} title={atmosphere?'关闭秋日氛围':'开启秋日氛围'}><Leaf/><span>秋意</span></Button><span className="return-deadline"><Clock3 size={15}/>10.03 · 11:00 长春还车</span></div></header>
    <div className="page-heading"><div><span className="eyebrow">大兴安岭 · 自驾路线</span><h1>沿着秋天，向北。</h1></div><div className="heading-note"><span className="verified-dot"/>百度路线已核验<small>09.20 酒店路线核算 · 非国庆实时路况</small></div></div>
    ...
    <section id="route-workspace" className="workspace" data-foliage={foliageForDay(dayId).kind} tabIndex={-1} aria-label="逐日地图与导航">
      <aside className="day-panel" aria-label="当天路线详情" style={leafVariables(dayId) as CSSProperties}>
        ...
        <div className="day-content" key={dayId}>
          ...
          <DailyStay dayId={dayId}/>
          <DrivingEffort dayId={dayId} minutes={minutes}/>
    ...
    <section className="trip-settings" aria-label="已确认行程条件"><div className="settings-title"><MapPinned size={17}/><div>4 人 · 已选定 7 晚住宿<small>9/27 美仑酒店交车 · 10/3 还车门店待确认</small></div></div><div className="settings-title"><Hotel size={16}/><span>10/1 乌兰浩特 · 10/2 长春净月</span></div><div className="total-distance"><strong>{Math.round(totalKm).toLocaleString()}<span>km</span></strong><small>当前酒店路线合计 · 不含园内游览</small></div></section>
    <BookedStays onDaySelect={selectLiteraryRoute}/>
...
<details className="data-note"><summary>地图与数据说明</summary><p>酒店接入路段于 2026 年 9 月 20 日通过百度地图核算；黑山头至 186、两旗之间沿用 9 月 9 日核验的草原路线。酒店终点采用地图导航点，实际停车入口与停车权限仍需向前台确认。默认连接 OpenStreetMap 详细底图，保留道路与地名，高分屏按像素密度加载更细瓦片。网络较慢时，随页面提供的 Natural Earth 河湖简图作为备用地理背景；它不含完整道路与行政边界。显示坐标从百度 BD-09 转为 WGS84，导航链接仍使用原始 BD-09。此页用于路线讨论，不替代行驶中的实时导航。时间不含休息、游览、补给及节假日额外拥堵。9/27 车辆送到美仑酒店，首段从酒店开始；交车时间与手续另计。10/3 仍以龙嘉机场 T2 为代表点，实际还车入口确定后复核。</p><p>每日头图按当日沿线区域或出发、到达城市切换。每张图附独立出处与历史时间，夏季照片会单独标明；照片不意味着已安排相应景点入园。10/1 已改为森林公园游览后住乌兰浩特，10/2 从全季到长春丽芮，不再使用阿尔山直返长春或松原过夜版本。</p><p>植物图形依据沿途景观选用杨叶、白桦叶、草原秋草和落叶松针簇，采用原创生成意象，不是各城市的专属标志或植物鉴定图。叶片颜色是依据区域景观设计的季节氛围，不是物候观测值、今年红叶预报或最佳观赏期保证；未把“入秋推迟”作为已核实的全线结论。草原、白桦、落叶松的变化不能用单一红叶百分比概括。满洲里到伊尔施按右旗、左旗锁定；直接在百度重新规划全段可能返回另一条路线。</p></details>
  </main>;
}
