...
export default function Home() {
  ...
  return <main className="atlas">
    ...
    <section className="trip-settings" aria-label="已确认行程条件"><div className="settings-title"><MapPinned size={17}/><div>4 人 · 已选定 7 晚住宿<small>9/27 取车接驳、10/3 还车门店待确认</small></div></div><div className="settings-title"><Hotel size={16}/><span>10/1 乌兰浩特 · 10/2 长春净月</span></div><div className="total-distance"><strong>{Math.round(totalKm).toLocaleString()}<span>km</span></strong><small>当前酒店路线合计 · 不含取车接驳与园内游览</small></div></section>
...
<details className="data-note"><summary>地图与数据说明</summary><p>酒店接入路段于 2026 年 9 月 20 日通过百度地图核算；黑山头至 186、两旗之间沿用 9 月 9 日核验的草原路线。酒店终点采用地图导航点，实际停车入口与停车权限仍需向前台确认。默认连接 OpenStreetMap 详细底图，保留道路与地名，高分屏按像素密度加载更细瓦片。网络较慢时，随页面提供的 Natural Earth 河湖简图作为备用地理背景；它不含完整道路与行政边界。显示坐标从百度 BD-09 转为 WGS84，导航链接仍使用原始 BD-09。此页用于路线讨论，不替代行驶中的实时导航。时间不含休息、游览、补给及节假日额外拥堵，实际租车门店确定后需重算接驳；9/27 暂示从美仑取到车后的酒店出发线，未将机场取车接驳算入。</p><p>每日头图按当日沿线区域或出发、到达城市切换。每张图附独立出处与历史时间，夏季照片会单独标明；照片不意味着已安排相应景点入园。10/1 已改为森林公园游览后住乌兰浩特，10/2 从全季到长春丽芮，不再使用阿尔山直返长春或松原过夜版本。</p><p>植物图形依据沿途景观选用杨叶、白桦叶、草原秋草和落叶松针簇，采用原创生成意象，不是各城市的专属标志或植物鉴定图。叶片颜色是依据区域景观设计的季节氛围，不是物候观测值、今年红叶预报或最佳观赏期保证；未把“入秋推迟”作为已核实的全线结论。草原、白桦、落叶松的变化不能用单一红叶百分比概括。满洲里到伊尔施按右旗、左旗锁定；直接在百度重新规划全段可能返回另一条路线。</p></details>
  </main>;
}
