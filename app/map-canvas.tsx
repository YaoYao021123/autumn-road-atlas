...
export default function MapCanvas({legs,allLegs,progress,overview,dayId,fallback,onOverview,scenes,showScenery,selectedSceneId,sceneSelectionKey,onSceneSelect}:Props) {
  ...
  const [detailed,setDetailed]=useState(true),[tileStatus,setTileStatus]=useState<'idle'|'loading'|'ready'|'slow'|'error'>('idle');
  ...
  useEffect(()=>{
    ...
    import('leaflet').then(L=>{
      ...
      const scenicPane=m.createPane('scenic-corridor');scenicPane.style.zIndex='350';scenicPane.style.pointerEvents='none';
  ...
  useEffect(()=>{
    ...
    const tiles=lib.current.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png',{
      maxZoom:19,keepBuffer:1,detectRetina:true,updateWhenIdle:true,attribution:'&copy; <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">OpenStreetMap</a>',
    });
    tiles.on('loading',()=>{
  ...
  useEffect(()=>{
    ...
    if(overview)allLegs.forEach(s=>L.polyline(s.points,{color:'#8c96a5',weight:3,opacity:.7,dashArray:'3 6',interactive:false}).addTo(layer));
    if(geometry.points.length) {
      // A white halo separates the route from roads/vegetation; blue is a
      // planning-state colour, never a fabricated traffic-speed classification.
      L.polyline(geometry.points,{color:'#fff',weight:12,opacity:1,interactive:false}).addTo(layer);
      L.polyline(geometry.points,{color:'#164695',weight:9,opacity:1,interactive:false}).addTo(layer);
      L.polyline(geometry.points,{color:'#2163d6',weight:6,opacity:1,interactive:false}).addTo(layer);
      trail.current=L.polyline([],{color:'#8291a5',weight:6,opacity:1,interactive:false}).addTo(layer);
    ...
    displayed.forEach((stop,i)=>{
      ...
      const marker=L.circleMarker(stop.position,{radius:active?6:3.5,color:'#fff',weight:2,fillColor:active?'#2163d6':'#8c96a5',fillOpacity:1}).addTo(layer);
  ...
  useEffect(()=>{
    ...
    if(showScenery)scenes.forEach(scene=>{
      if(scene.highlight.length>1)L.polyline(scene.highlight,{pane:'scenic-corridor',color:'#bf871b',weight:17,opacity:.8,dashArray:'2 11',lineCap:'round',interactive:false}).addTo(layer);
      const isWindow=scene.kind==='window';
      const ordinal=scenes.filter(s=>s.day===scene.day).findIndex(s=>s.id===scene.id)+1;
      const icon=L.divIcon({className:`scenery-marker ${isWindow?'window-marker':'parking-marker'}`,html:scenerySymbol(isWindow,ordinal,scene.day),iconSize:isWindow?[38,43]:[29,29],iconAnchor:isWindow?[19,39]:[14,36],popupAnchor:[0,-33]});
  ...
  return <>
    ...
    <div className="map-route-key" aria-label="蓝线为规划路线，灰线为已回放路段，不表示实时路况"><i/>规划路线<em/>已回放<small>非实时路况</small></div>
